...
sleep 2
        echo -e "${CLR_BLD_BLU}             ooooooooooo              ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}         ooooooooooooooooooo          ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}      oooooooo   oooo   oooooooo      ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}    ooooooo      oooo      ooooooo    ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}   oooooo        oooo        oooooo   ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}  oooooo         oooo         oooooo  ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}  ooooo          oooo          ooooo  ${CLR_RST}"
        echo -e "${CLR_BLD_BLU} oooooo         oooooo         oooooo ${CLR_RST}"
        echo -e "${CLR_BLD_BLU} oooooo        oooooooo        oooooo ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}  ooooo      oooooooooooo      ooooo  ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}  oooooo   oooo  oooo  oooo   oooooo  ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}   oooooooooo    oooo    oooooooooo   ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}    oooooooo     oooo     oooooooo    ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}      oooooooo   oooo   oooooooo      ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}         oooooooooooooooooooo         ${CLR_RST}"
        echo -e "${CLR_BLD_BLU}             oooooooooooo             ${CLR_RST}"
            echo -e ""
            echo -e "${CLR_BLD_BLU}================================${CLR_RST}"
            echo -e "${CLR_BLD_BLU}          FredProject           ${CLR_RST}"
            echo -e "${CLR_BLD_BLU}================================${CLR_RST}"
            echo -e ""
sleep 2
...
if [ ! -d "$DIR_ROOT/vendor/fred" ]; then
...
FRED_VERSION=$(cat $DIR_ROOT/vendor/fred/config/common.mk | grep 'FRED_VERSION := *' | sed 's/.*= //')
...
echo -e "${CLR_BLD_GRN}Building Fred $FRED_VERSION for $DEVICE${CLR_RST}"
...
if [ "$FLAG_USER_BUILD" = 'y' ]; then
        lunch "fred_$DEVICE-user"
else
        lunch "fred_$DEVICE-userdebug"
fi
echo -e ""
